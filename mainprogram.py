try:
    ...
    if languageconfiguration == "sv-se":
        ...
        initialspeedtestinformation = "För att få programmet att fungera optimalt så körs ett eller flera \"speedtest(s)\" för att mäta tid m.m. Det finns olika alternativ för detta."
        ...
        superbmodedescription = "SUPERB - Fem speedtests komemr att köras, vilket bara rekommenderas när du behöver en exakt tidsavläsning. Detta läge avrundar tidsdata till en decimal, vilket är mer exakt än andra lägen. Detta läge kommer dock att ta två minuter att köra."
        ...
        initialspeedtestinformation = "To make the program work better, one or more \"speedtest(s)\" is being run in the beginning of the program. You can choose between different modes:"
    ...
    if speedtestaccuracymode == "NORMAL":
        timeone = time.time()
        if noconnection == 0:
    ...
    elif speedtestaccuracymode == "ACCURATE":
        timelist = []
        if noconnection == 0:
            for i in range(3):
                time1 = time.time()
                processpeedtest(False, "initialspeedtest.txt", textlayout="Text")
                timelist.append(round(time.time()-time1))
                print(speedtestran + str(timelist[i]) + speedtestseconds)
            speedtesttime = round(eval("+".join(timelist)))
        elif noconnection == 1:
            print(nointernetconnectionmessage)
    elif speedtestaccuracymode == "FAST":
        timelist3 = []
        if noconnection == 0:
            timeone1 = time.time()
            processspeedtest_onlyupload("initialspeedtests.txt")
            speedtesttime = round((time.time() - timeone1)*2)
            print(speedtestran + str(round(speedtesttime/2)) + speedtestseconds + " " + fastspeedtest)

    elif speedtestaccuracymode == "SUPERB":
        timelist2 = []
        if noconnection == 0:
            for i in range(5):
    ...
    if mode == "BETA MODE":
        ...
        if noconnection == 0:
            for speedtestindex in range(repeat):
                try:
                    print(startingspeedtestmessage)
                    processpeedtest(0, filename=filedirectory, textlayout="Text")
                    print(speedtestcompletedmessage)
